...
class DynamicNN1d(NNFrameWork):
    ...
    def set_step_optimizer_golf(self, lr=1, beta=0.5):
        self.optimizer = GOLF(self.layers[-1].parameters(), beta=beta, lr=lr)
...
def dyna_train_golf(net, a1, a2, hidden_size, data_iter, lr, beta, epochs, x_to_fit, y_to_fit):
    """
    动态训练神经网络(训练时仅训练单层)
    :param net: 网络
    :param a1: 激活函数1
    ...
    """
    net.add_layer(hidden_size, a1, a2)
    net.set_step_optimizer_golf(lr=lr, beta=beta)
    net.train_fixed(data_iter, epochs=epochs, plot_hist=True)
    plot_fitting_result(net, x_to_fit, y_to_fit)
    return net.loss_history[-1]
...
if __name__ == "__main__":
    ...
    dyna_nn.set_step_optimizer_golf(lr=1, beta=0.5)
    ...
    # # 动态神经网络第一次训练
    dyna_train_golf(dyna_nn, LaplaceActivation, LaplaceActivation, 10, data_iter, 0.1, 0.5, 1000, x, y)
