...
class NNFrameWorkV2(nn.Module):
    ...
    def _self_update(self, data=None, target=None):
        if self.optim_mode == "sgd":
            if self.clip_value is not None:
                nn.utils.clip_grad_norm_(self.parameters(), self.clip_value)  # 梯度截断
            self.optimizer.step()
        elif self.optim_mode == "tsgd":
            self.optimizer.step(lambda: self.criterion(self(data), target))
    ...
    def set_optimizer_tsgd(self, lr_start=1.0, lr_k=0.1, lr_num=3, soft_p=9):
        self.optim_mode = "tsgd"
        self.optimizer = TentativeSGD(self.parameters(),
