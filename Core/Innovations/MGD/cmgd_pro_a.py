        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if len(state) == 0:
                    ...
                    state['lr_grad_var'] = nn.init.constant_(
                        torch.empty_like(
                            p.data, memory_format=torch.preserve_format
                        ), 0
                    )
                ...
                new_lr = torch.where(mask, torch.abs(clamped_weight_change / grad),
                                     state['lr'])  # 当mask对应部分为true(被截断)，则使用调整后的学习率，否则使用原学习率
                state['lr'] = new_lr
    ...
    def step_follow(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                # 计算二阶矩估计
                state['lr_grad_var'] = group['beta2'] * state['lr_grad_var'] + (1 - group['beta2']) * (lr_grad ** 2)
                state['step'] += 1
                bias_correction2 = 1 - group['beta2'] ** state['step']
                lr_grad_var_corr = state['lr_grad_var'] / bias_correction2

                # 放大学习率梯度
                lr_grad = lr_grad / (torch.sqrt(lr_grad_var_corr) + group['eps'])
