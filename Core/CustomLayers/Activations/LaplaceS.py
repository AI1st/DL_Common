...
class Snake(nn.Module):
    def __init__(self, features):
        ...
        self.a1 = nn.Parameter(10 * torch.zeros(features))
        self.b1 = nn.Parameter(torch.ones(features))
        self.a2 = nn.Parameter(10 * torch.zeros(features))
        self.b2 = nn.Parameter(-torch.ones(features))

    def forward(self, x):
        return x - self.a1 * torch.clamp(x, min=torch.tensor(0).to(self.a1.device),
                                         max=self.b1) - self.a2 * torch.clamp(x, min=-self.b2,
                                                                              max=torch.tensor(
                                                                                  0).to(self.a1.device))


class LaplaceActivationS(nn.Module):
    """
...
if __name__ == "__main__":
    ...
    def plot_laplace_activation(parameters):
        ...
        for idx, (a_value, b_value) in enumerate(parameters):
            model = LaplaceActivationS(features=1, bias=(b_value is not None))
